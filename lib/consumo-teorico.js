...
//
// BACON/GORGONZOLA/TOMATE/CEBOLA: mesma fonte e lógica dos insumos acima.
//
// CALABRESA (insumo real, carne/embutido): a ficha da Calabresa também serve
// de fallback de CUSTO para sabores sem ficha própria (Predileta, Palmito,
// Champignon etc — ver FALLBACK_SALGADO). Esses sabores não usam calabresa de
// verdade, então o KPI de consumo real de calabresa exclui pizzas marcadas
// como fallback; o volume aproximado por fallback fica separado em
// calabresaFallbackGramas, só para referência de custo.
 ...
 *   calabresaFallbackGramas: number, calabresaFallbackKg: number,
 *   catupiryGramas: number, catupiryKg: number,
 *   frangoGramas: number, frangoKg: number,
 *   presuntoGramas: number, presuntoKg: number,
 *   baconGramas: number, baconKg: number,
 *   gorgonzolaGramas: number, gorgonzolaKg: number,
 *   tomateGramas: number, tomateKg: number,
 *   cebolaGramas: number, cebolaKg: number,
...
function computeConsumoTeorico(salesItemsResponse) {
  ...
  let calabresaFallbackGramas = 0;
  let catupiryGramas = 0;
  let frangoGramas = 0;
  let presuntoGramas = 0;
  let baconGramas = 0;
  let gorgonzolaGramas = 0;
  let tomateGramas = 0;
  let cebolaGramas = 0;
  ...
  for (const sale of salesItemsResponse) {
    ...
    for (const item of sale.items) {
      ...
      processed.pizzas30cm.forEach(p => {
        ...
        if (p.fallback) {
          calabresaFallbackGramas += CALABRESA_GRAMAS[p.fichaTecnica] ?? 0;
        } else {
        ...
        baconGramas += BACON_GRAMAS[p.fichaTecnica] ?? 0;
        gorgonzolaGramas += GORGONZOLA_GRAMAS[p.fichaTecnica] ?? 0;
        tomateGramas += TOMATE_GRAMAS[p.fichaTecnica] ?? 0;
        cebolaGramas += CEBOLA_GRAMAS[p.fichaTecnica] ?? 0;
      });
  ...
  return {
    ...
    calabresaFallbackGramas,
    calabresaFallbackKg: calabresaFallbackGramas / 1000,
    catupiryGramas,
    ...
    baconGramas,
    baconKg: baconGramas / 1000,
    gorgonzolaGramas,
    gorgonzolaKg: gorgonzolaGramas / 1000,
    tomateGramas,
    tomateKg: tomateGramas / 1000,
    cebolaGramas,
    cebolaKg: cebolaGramas / 1000,
