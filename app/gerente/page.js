...
export default function Gerente() {
  ...
  const totalPendente = freelancers.filter(f => !f.pago).reduce((acc, f) => acc + parseFloat(f.valor), 0)
  const totalPago = freelancers.filter(f => f.pago).reduce((acc, f) => acc + parseFloat(f.valor), 0)
              ...
              <div key={f.id} style={st.card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 700, color: '#1a1a18', marginTop: 4 }}>R$ {parseFloat(f.valor).toFixed(2)}</div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' }}>
                    <button onClick={() => togglePago(f.id, f.pago)} style={{ padding: '6px 12px', borderRadius: 8, border: 'none', background: f.pago ? '#eaf3de' : '#fcebeb', color: f.pago ? '#3b6d11' : '#a32d2d', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                      {f.pago ? '✅ Pago' : '⏳ Pendente'}
                    </button>
                    <button onClick={() => deletarFreelancer(f.id)} style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid #e5e5e0', background: '#fff', color: '#888780', fontSize: 11, cursor: 'pointer' }}>
                      🗑️ Remover
                    </button>
                  </div>
                </div>
