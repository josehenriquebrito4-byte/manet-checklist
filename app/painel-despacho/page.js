...
const ALERTA_FALTAM_MINUTOS = 5 // muda de cor quando faltar isso pro pedido sair do painel

function elapsedLabel(readyAtIso) {
  const ms = Date.now() - new Date(readyAtIso).getTime()
...
function Coluna({ titulo, cor, corTexto, corCardBg, corCardBorda, pedidos, janelaMinutos }) {
  const st = {
    ...
    saleNumber: { fontSize: 40, fontWeight: 800, lineHeight: 1 },
    ...
    tempo: { fontSize: 52, fontWeight: 800, fontVariantNumeric: 'tabular-nums', flexShrink: 0 },
  }
