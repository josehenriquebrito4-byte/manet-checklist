...
const TAREFAS = [
  ...
  { key: 't2', label: 'Máquinas carregadas' },
  { key: 't3', label: 'Cardápios Saipos/iFood/99Food batido' },
  { key: 't4', label: 'Tempo de entrega em 9' },
  { key: 't5', label: 'Status verificado' },
  { key: 't6', label: 'Projeção de parcial pedidos na TV' },
  { key: 't7', label: 'Colocar status e parcial de 30 em 30 min' },
  { key: 't8', label: 'Olhar grupo de controle — algum informe' },
  { key: 't9', label: 'Fazer controle feedback e salvar ruins no bloco de notas' },
  { key: 't10', label: 'Preencher o COP até as 17h' },
  { key: 't11', label: 'Informar líder cozinha projeção do dia' },
  { key: 't12', label: 'Informar pelo WhatsApp no grupo COP o OK do formulário' },
]

export default function ChecklistAtendente() {
  ...
  const handleEnviar = async () => {
    ...
    try {
      await fetch('/api/checklist', {
        ...
        body: JSON.stringify({ nome, tipo: 'atendente', turno: 'abertura', tarefas, resultados: {} })
      })
      setConcluido(true)
    } catch (e) {}
    setEnviando(false)
  ...
  if (concluido) return (
    <div style={st.wrap}>
      <div style={st.header}>
        ...
        <h1 style={st.title}>Checklist Atendente</h1>
      </div>
